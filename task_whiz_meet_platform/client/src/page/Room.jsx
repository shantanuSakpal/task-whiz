...
const Room = () => {
	...
	useEffect(() => {
		...
		const unsub = () => {
			socket.current = io.connect("https://task-whiz-socket.onrender.com");
	...
	const handleEndCall = async () => {
		...
		try {
			const response = await fetch("http://taskwhiz.socket.onrender.com/getTasks");
						...
						user && (
							<motion.div layout className="flex flex-row bg-darkBlue2 text-white w-full">
								<motion.div layout className="flex flex-col bg-darkBlue2 justify-between w-full">
									<div
										className="flex-shrink-0 overflow-y-scroll bg-[#202124] flex p-3"
										style={{
									...
									>
										<motion.div
											...
											className={`grid grid-cols-1 gap-4  ${
												showChat ? "md:grid-cols-1 md:mr-4" : "lg:grid-cols-3 sm:grid-cols-2"
											} `}
										>
											<motion.div
												layout
										...
										<div>
											<iframe
												className="w-[20rem] h-full mx-auto"
												src="https://sign.mt/?embed=&amp;spl=en&amp;sil=ase&amp;text=Hello%20world!"
												// src={`https://sign.mt/?embed=&amp;spl=en&amp;sil=ase&amp;text=${"Hello World"}`}
